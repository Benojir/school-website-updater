...
if (!isset($_GET['admit_id']) || empty($_GET['admit_id'])) {
    die("Admit Card ID is required");
}

$admit_id = $_GET['admit_id'];

// Accept student IDs as comma-separated list or array
$student_ids = [];
if (isset($_GET['student_ids']) && !empty($_GET['student_ids'])) {
    $student_ids = is_array($_GET['student_ids']) ? $_GET['student_ids'] : explode(',', $_GET['student_ids']);
...
$pdf->SetTitle('Bulk Admit Cards - ' . $admit_id);
$pdf->SetSubject('Student Admit Cards');
...
// Counter to track card position on page
$card_position = 0;
...
foreach ($student_ids as $student_id) {
    ...
    // Add new page if this is the first card or we've already placed 2 cards on current page
    if ($card_position % 2 == 0) {
        $pdf->AddPage();
        $current_y = $margin_top;
    } else {
        $current_y = $margin_top + $card_height + $gap_between_cards;
    }

    // Create admit card
    createAdmitCard($pdf, $student, $admit_card, $exam_routine, $current_y, $card_width, $card_height, $school_name, $school_address);

    $card_position++;
}

// Output the PDF
$pdf->Output('Bulk_AdmitCards_' . $admit_id . '.pdf', 'D');
...
function createAdmitCard($pdf, $student, $admit_card, $exam_routine, $start_y, $width, $height, $school_name, $school_address)
{
    ...
    // Signatures - compact
    $pdf->SetXY(10, $start_y + $height - 10);
