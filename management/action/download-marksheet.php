...
$stmt = $pdo->prepare("
    ...
    WHERE f.student_id = ?
    GROUP BY sp.override_marksheet_check, sp.allow_marksheet;
");
$stmt->execute([$result['student_id']]);
$permissions = $stmt->fetch(PDO::FETCH_ASSOC);

// Default behavior - check fees if not overridden
if (!$permissions || !$permissions['override_marksheet_check']) {
    if (($permissions['unpaid_amount'] ?? 0) > 0) {
        die("Marksheet download is not permitted - pending fees must be cleared first");
    }
}
// If overridden, check explicit permission
else {
    if (!$permissions['allow_marksheet']) {
        die("Marksheet download has been disabled for this student");
    }
}

// Fetch subject marks (updated query)
$stmt = $pdo->prepare("
...
$pdf->SetTitle('Marksheet - ' . $result['name']);
...
$pdf->SetAlpha(0.1);
$pdf->SetFillColor(0, 0, 255); // White background
$pdf->Rect(0, 0, $pdf->getPageWidth(), $pdf->getPageHeight(), 'F'); // Fill the entire page with white


// Add watermark (semi-transparent school logo in center)
$watermarkPath = '../../uploads/school/logo-square.png'; // Same as your logo path
if (file_exists($watermarkPath)) {
    // Get original image dimensions
    list($width, $height) = getimagesize($watermarkPath);

    // Calculate size to fit page (about 50% of page width)
    $maxWidth = 100; // mm
    $ratio = $width / $height;
...
$logoPath = $watermarkPath; // Update this path
...
foreach ($subject_marks as $subject) {
    $subject_tbl .= '<tr>
        <td align="center">' . htmlspecialchars($subject['subject_name']) . '</td>
        <td align="center">' . htmlspecialchars($subject['subject_type']) . '</td>
...
// Fetch grading system data from database
$pdf->setTextColor(0, 0, 0);
...
// Generate QR code (positioned to the right of grading table)
...
// Output the PDF
$pdf->Output('Marksheet_' . $result['student_id'] . '_' . $result['exam_name'] . '.pdf', 'D');
